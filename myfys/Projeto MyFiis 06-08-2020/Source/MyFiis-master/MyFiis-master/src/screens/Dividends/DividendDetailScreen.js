...
class DividendDetailScreen extends React.Component {
  constructor(props) {
    super(props);
  ...
  checkCanSubmit() {
    let total = this.state.total
      ? parseInt(this.state.total.toString().replace(/[^0-9]/g, ""))
      : 0;
    ...
    if (
      ...
      this.state.canSubmit &&
      (total < 1 || !this.state.date || !this.state.fii || quantity < 1)
    ) {
      this.setState({
        canSubmit: false,
      });
  ...
  async onSubmit() {
    ...
    let total = parseInt(this.state.total.toString().replace(/[^0-9]/g, ""));
    ...
    try {
      if (this.props.route.params.action == "edit") {
        this.props.firebase.updateDividend({
        ...
        this.props.firebase.addDividend({
          fii: fiis_code,
          total: total,
          date: year + "-" + month + "-" + day,
          quantity: quantity,
  ...
  async onDelete() {
    try {
      if (this.props.route.params.action == "edit") {
        await this.props.firebase.deleteDividend({
          id: this.props.route.params.editObj.id,
        });
      }
    } catch (e) {
      console.log("Error onDelete Dividend", e);
    }
      ...
      <View style={{ backgroundColor: "#FAFAFA", flex: 1 }}>
            ...
            <View style={{ paddingHorizontal: 20, paddingTop: 50, flex: 1 }}>
              ...
              <View style={styles.row}>
                <SearchableDropdown
                  ...
                  itemsContainerStyle={{ maxHeight: 120, borderWidth: 0 }}
                  items={this.state.fiisList}
                  defaultIndex={0}
                  resetValue={false}
                  textInputProps={{
                    //Se usar value buga quando digita rápido
                ...
                <TextInputMask
                  ...
                  onChangeText={this.onChangeTotal.bind(this)}
                  placeholder={"Total"}
                  type={"money"}
                  autoCorrect={false}
        ...
        {this.state.showConfirmDelete ? (
          <AwesomeAlert
            ...
            showProgress={false}
            title={"Atenção"}
            message={"Confirma a exclusão desse dividendo?"}
...
export default withFirebase(function (props) {
  ...
  return <DividendDetailScreen {...props} navigation={navigation} />;
});
