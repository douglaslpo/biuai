<template>
  <v-dialog v-model="dialog" max-width="600px">
    ...
    <v-card>
      <v-card-title>
        <span class="text-h5">Adicionar Novo FII</span>
      </v-card-title>
...
const { addFIIToPortfolio } = useMyFIIs()
...
const handleSubmit = async () => {
  ...
  try {
    loading.value = true
    await addFIIToPortfolio(fiiData)
    showSuccess('FII adicionado com sucesso!')
    dialog.value = false
    resetForm()
  } catch (error) {
...
const resetForm = () => {
  form.value?.reset()
  Object.keys(fiiData).forEach(key => {
    if (typeof fiiData[key] === 'number') {
      fiiData[key] = null
    } else {
      fiiData[key] = ''
    }
